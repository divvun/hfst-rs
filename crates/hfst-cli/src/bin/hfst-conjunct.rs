    ...
    unsafe {
        ...
        loop {
            ...
            long_options.push(getopt::Option {
                name: CString::new("harmonize-flags").unwrap().into_raw(),
                has_arg: 0,
    ...
    unsafe {
        ...
        if type1 != type2 {
            if globals::ALLOW_TRANSDUCER_CONVERSION {
                ...
                if ct == 1 {
                    ...
                    // should not happen
                    std::panic::panic_any(
                        "Error: hfst-conjunct: conversion_type returned an invalid integer",
                    );
                }
                warning(0, 0, &warnstr);
            } else {
                error(
                    ...
                    0,
                    &format!(
                        "Transducer type mismatch in {} and {}; formats {} and {} are not compatible for conjunction (--do-not-convert was requested)",
                        cstr(globals::FIRSTFILENAME),
                        cstr(globals::SECONDFILENAME),
        ...
        while continue_reading {
            ...
            let secondname = {
                ...
                hfst_get_name(second_t, &cstr(globals::SECONDFILENAME))
            };
...

            {
                ...
                if first_t.has_flag_diacritics() || second_has_flags {
                    if !HARMONIZE_FLAGS {
                        if !globals::SILENT {
                            warning(
                                0,
            ...
            // C: hfst_set_name(*first, *first, *second, "intersect"); the dest
...
unsafe fn real_main() -> c_int {
    unsafe {
        ...
        conjunct_streams(&mut firststream, &mut secondstream)
    }
}
