...
// (and rightmost) functions on single rules. The restriction and before rules,
// parallel rules, and epenthesis rules are tested in
// test_xerox_rules_restriction.rs, test_xerox_rules_parallel.rs and
// test_xerox_rules_epenthesis.rs. The notes below apply to all four files.
//
...
fn test10a<B: AlgebraBackend>() -> Result<(), hfst::error::Error> {
    ...
    result1.insert_to_alphabet_string("a")?;
