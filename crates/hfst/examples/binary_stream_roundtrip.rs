...
use hfst::transducer::Transducer;
...
// The HFST-OL output stream is deferred, so assemble a valid HFST-OL file by
// hand: the same HFST header the writer emits (HFST\0 + 2-byte length + \0 +
// "version\03.3\0type\0HFST_OL[W]\0") followed by the optimized-lookup payload
// 'Transducer::write' produces.
fn read_hfst_ol(weighted: bool, label: &str) {
    let type_ = if weighted {
        ImplementationType::HFST_OLW_TYPE
    } else {
        ImplementationType::HFST_OL_TYPE
    };
    let type_str = if weighted { "HFST_OLW" } else { "HFST_OL" };

    let mut payload: Vec<u8> = Vec::new();
    Transducer::new_weighted(weighted).write(&mut payload);

    let mut header_data: Vec<u8> = Vec::new();
    for s in ["version", "3.3", "type", type_str] {
        header_data.extend_from_slice(s.as_bytes());
        header_data.push(0);
    }
    let len = header_data.len() as u16;
    let lb = len.to_le_bytes();

    let mut file: Vec<u8> = Vec::new();
    file.extend_from_slice(b"HFST");
    file.push(0);
    file.push(lb[0]);
    file.push(lb[1]);
    file.push(0);
...
fn main() {
    ...
    read_hfst_ol(false, "hfst_ol");
    read_hfst_ol(true, "hfst_olw");
    read_att_facade();
